    ...
    return (
        <div className="p-6 pt-8 font-body" data-testid="messages-page">
            <div className="flex items-center gap-2 mb-6">
                <MessageCircle className="w-6 h-6 text-[#E3FF00]" />
            ...
            <div className="space-y-2">
                {convs.map((c) => (
                    <Link
                        ...
                        className="flex items-center gap-3 bg-[#141414] border border-[#262626] hover:border-[#E3FF00] rounded-2xl p-3 transition"
                    >
                        <div className="w-12 h-12 rounded-full bg-[#E3FF00] flex items-center justify-center text-black font-heading font-black">{c.user.name?.[0]}</div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                                <div className="font-heading font-bold">@{c.user.username}</div>
                                {c.unread > 0 && <span className="bg-[#E3FF00] text-black text-xs font-bold rounded-full px-2 py-0.5">{c.unread}</span>}
                            </div>
