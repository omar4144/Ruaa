...
export default function MarketplaceDetail() {
    ...
    return (
        <div className="p-6 pt-8 font-body pb-24" data-testid="request-detail">
            ...
            <span className="text-xs bg-[#E3FF00]/20 text-[#E3FF00] px-2 py-1 rounded-full">{req.category}</span>
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
                <div className="bg-[#141414] border border-[#262626] rounded-xl p-3 text-center">
                    <DollarSign className="w-4 h-4 text-[#E3FF00] mx-auto mb-1" />
                    <div className="text-sm font-heading font-bold">${req.budget_min}-${req.budget_max}</div>
                </div>
                <div className="bg-[#141414] border border-[#262626] rounded-xl p-3 text-center">
                    <Clock className="w-4 h-4 text-[#E3FF00] mx-auto mb-1" />
                    <div className="text-sm font-heading font-bold">{req.deadline_days} يوم</div>
                </div>
                <div className="bg-[#141414] border border-[#262626] rounded-xl p-3 text-center">
                    <Users className="w-4 h-4 text-[#E3FF00] mx-auto mb-1" />
            ...
            {!isOwner && (
                <form onSubmit={apply} className="bg-[#141414] border border-[#262626] rounded-2xl p-4 mb-6">
                    ...
                    <textarea required data-testid="apply-msg" placeholder="لماذا أنت الشخص المناسب؟" rows={3} value={msg} onChange={(e) => setMsg(e.target.value)} className="w-full bg-black border border-[#262626] rounded-xl px-4 py-3 focus:border-[#E3FF00] focus:outline-none resize-none mb-3" />
                    <input type="number" min="0" data-testid="apply-price" placeholder="سعرك المقترح ($)" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full bg-black border border-[#262626] rounded-xl px-4 py-3 focus:border-[#E3FF00] focus:outline-none mb-3" />
                    <button data-testid="apply-btn" type="submit" className="w-full bg-[#E3FF00] text-black font-heading font-bold rounded-full py-3">إرسال العرض</button>
                </form>
            )}
                            ...
                            <div key={a.id} className="bg-[#141414] border border-[#262626] rounded-2xl p-3" data-testid={`app-${a.id}`}>
                                <div className="flex items-center justify-between mb-2">
                                    <Link to={`/u/${a.user?.username}`} className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-full bg-[#E3FF00] flex items-center justify-center text-black font-heading font-black text-xs">{a.user?.name?.[0]}</div>
                                        <div className="text-sm font-heading font-bold">@{a.user?.username}</div>
                                    </Link>
                                    {a.proposed_price > 0 && <span className="text-[#E3FF00] font-heading font-black">${a.proposed_price}</span>}
                                </div>
                                <p className="text-sm text-neutral-300">{a.message}</p>
                                <Link to={`/messages/${a.user?.username}`} className="text-xs text-[#E3FF00] mt-2 inline-block">تواصل →</Link>
                            </div>
                        ))}
                    </div>
                </div>
